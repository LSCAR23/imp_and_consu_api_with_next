    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (data.qrCode) {
                ...
                setMessage('Registro exitoso. Escanea el código QR para habilitar 2FA.');
                setFormData({ email: '', userName: '', password: '' });
            } else {
                setMessage('Registro exitoso, pero no se recibió el código QR.');
            }
        } catch (error) {
            ...
            {qrCode && (
                <div className="fixed inset-0 bg-black bg-opacity-75 backdrop-blur-lg flex justify-center items-center z-50">
                    <div className="bg-gray-800 p-6 rounded-lg text-center shadow-lg shadow-violet-500 max-w-sm w-full">
                        ...
                        <button onClick={redirectToLogin} className="mt-4 bg-violet-500 text-white py-2 px-4 rounded-lg hover:bg-violet-600 transition duration-300" > Ir al Login </button>
                    </div>
                </div>
            )}
        </div>
